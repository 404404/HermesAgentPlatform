...
export function ActionCell({
  ...
  return <Space className="table-actions" size={4}>
    {onEnabledChange && <Tooltip title={t('status')}><Switch size="small" checked={enabled} loading={loading} onChange={onEnabledChange} /></Tooltip>}
    ...
    {moreItems?.length ? <Dropdown menu={{ items: moreItems }} trigger={['click']}><Tooltip title={t('moreActions')}><Button size="small" type="text" icon={<MoreOutlined />} aria-label={t('moreActions')} /></Tooltip></Dropdown> : null}
  </Space>
}
