  ...
  return <Card className="content-card" title={t('runtimeInfrastructure')} extra={<Button type="primary" icon={<PlusOutlined />} onClick={() => edit()}>{t('addRuntimeHost')}</Button>}>
    ...
    <Tabs items={[
      { key: 'hosts', label: t('runtimeHosts'), children: <Table rowKey="id" dataSource={hosts} scroll={{ x: 1450 }} columns={[
        ...
        { title: t('authentication'), render: (_: unknown, row: any) => t('enum.' + (row.credential_status || (row.credential_reference_configured ? 'configured' : 'missing')), row.credential_status || (row.credential_reference_configured ? 'configured' : 'missing')) },
    ...
    <Drawer title={detail?.name || t('runtimeHost')} open={!!detail} onClose={() => setDetail(undefined)} width={760}>{detail && <Tabs items={[
      { key: 'overview', label: t('overviewTab'), children: <Descriptions column={2} items={[{ key: 'address', label: t('hostIp'), children: detail.address }, { key: 'status', label: t('status'), children: <HostStatus value={detail.status} /> }, { key: 'auth', label: t('authentication'), children: t('enum.' + (detail.credential_status || (detail.credential_reference_configured ? 'configured' : 'missing')), detail.credential_status || (detail.credential_reference_configured ? 'configured' : 'missing')) }, { key: 'docker', label: 'Docker', children: detail.docker_version }]} /> },
