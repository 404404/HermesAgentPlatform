...
Settings → Runtime Infrastructure is the physical/VM host onboarding surface. A Runtime Host holds address, SSH port and username, Docker local socket path, inventory and a credential reference. The write-only SSH password is bcrypt-protected in `secrets.encrypted_value`; no Runtime Host response exposes a password, credential identifier or Docker socket. The `MockRuntimeHostProvider` returns deterministic SSH/Docker/resource checks in this Demo and never opens SSH or Docker.
