...
func (s *server) listRuntimesV2(c *gin.Context) {
	...
	rows, err := s.db.Query(`SELECT r.id,r.user_id,u.display_name,r.runtime_id,r.status,r.desired_status,r.observed_status,r.provider,r.hermes_version,COALESCE(h.name,''),r.host_id,
		...
		r.template_id,r.created_at,r.last_seen FROM runtimes r JOIN users u ON u.id=r.user_id LEFT JOIN runtime_hosts h ON h.id=r.host_id ORDER BY r.created_at DESC`)
	if err != nil {
	...
	for rows.Next() {
		...
		var user, rid, status, desired, observed, provider, version, hostName, cpu, memory, storage, image, runtimeProvider, class, network, created string
		var autoStart, autoSuspend bool
		var templateID, hostID sql.NullInt64
		var last sql.NullTime
		var profiles int
		if err := rows.Scan(&id, &uid, &user, &rid, &status, &desired, &observed, &provider, &version, &hostName, &hostID, &profiles, &cpu, &memory, &storage, &profileLimit, &jobs, &image, &runtimeProvider, &class, &network, &autoStart, &autoSuspend, &templateID, &created, &last); err != nil {
			continue
		}
