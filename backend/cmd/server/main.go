...
func main() {
	...
	if cfg.userPassword, err = demoSeedPassword("HEP_DEMO_USER_PASSWORD", cfg.demoMode, "ChangeMe-User-2026!"); err != nil {
		log.Fatal(err)
	}
	...
	v1.GET("/auth/demo-info", s.demoInfo)
	auth := v1.Group("")
	...
	auth.POST("/skills/:id/submit", s.submitSkillV033Hotfix)
	auth.GET("/skill-submissions", s.listSubmissions)
	auth.POST("/skill-submissions/:id/review", s.legacySkillReviewV033Hotfix)
...
// demoSeedPassword permits a documented local Demo credential only when Demo mode is explicit.
// Password hashes are generated by the seed path; no plaintext is stored in the database.
func demoSeedPassword(key string, demoMode bool, demoFallback string) (string, error) {
	if value := strings.TrimSpace(os.Getenv(key)); value != "" {
		return value, nil
	}
	if demoMode {
		log.Printf("DEMO MODE: %s is not set; using the documented local Demo password", key)
		return demoFallback, nil
	}
	return "", fmt.Errorf("%s must be set when HEP_DEMO_MODE is false", key)
}
...
func (s *server) demoInfo(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{"data": gin.H{"demo_mode": s.cfg.demoMode}})
}
